...
export default {
  ...
  methods: {
    ...
    change_image(i){
      ...
      for(var index = 0; index < up_image.files.length; index++){
                htl+=`<img style="width:100px;height:100px;margin-left:10px" src="${URL.createObjectURL(up_image.files[index])}"></img>`;
            }
            if(up_image.files.length>3){
              alert("最多可传3张图片");
            }else{
              image_box.innerHTML = htl;
            }
...
#m-comment .commentPicturl{
  position: relative;
   width: 100%;
   border-bottom: .024155rem solid #c1c1c1;
   padding-bottom: .483092rem;
}
#m-comment .addPicturl span {
  font-size: 1.207729rem;
  margin: 0 auto;
}
#m-comment .addPicturl p {
  font-size: 0.410628rem;
  color: #676565;
}
#m-comment .commentPicturl input { 
   opacity: 0;
  width: 2.415459rem;
  height: 2.415459rem;
  position: absolute;
